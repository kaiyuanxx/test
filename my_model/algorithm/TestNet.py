...
class TestNet(SLModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if unlabeled_mask.sum() > 0:
            ...
            if self.current_epoch >= 50 and low_conf_mask.sum() > 1:
    ...
    def test_step(self, batch, batch_idx):
        ...
        x_recon = self.encoder.decode(z_sem, z_forg)
        # 仅语义重构：z_forg置零
        z_forg_zero = torch.zeros_like(z_forg)
        recon_sem_only = self.encoder.decode(z_sem, z_forg_zero)

        # 仅伪造重构：z_sem置零
        z_sem_zero = torch.zeros_like(z_sem)
        recon_forg_only = self.encoder.decode(z_sem_zero, z_forg)

        if batch_idx < 5: 
            self.save_reconstruction_results(images, x_recon, recon_forg_only, recon_sem_only, batch_idx)
    ...
    def save_reconstruction_results(self, real_img, recon_img, recon_forg_only, recon_sem_only, batch_idx):
        """
        保存重构对比图：原图 | 重构图
        """
        ...
        recon_forg_only = denormalize(recon_forg_only)
        recon_sem_only = denormalize(recon_sem_only)
        ...
        comparison_list = torch.cat([
            ...
            recon_forg_only[:n_samples].cpu(),
            recon_sem_only[:n_samples].cpu()
        ])
        
        # 生成网格图 (每行 3 张图)
        grid = torchvision.utils.make_grid(comparison_list, nrow=n_samples, normalize=False, padding=3, pad_value=1.0)
